...
class Learner_SuEIrD(Model):
    def __init__(self, N, E_0, I_0, D_0):
        self.N = N
        self.E_0 = E_0
        self.I_0 = I_0
        self.D_0 = D_0
    ...
    def __call__(self, size, params, init):
        ...
        def calc_grad(t, y):
            ...
            return [-beta*S*(E+I)/self.N, beta*S*(E+I)/self.N-sigma*E, mu*E-gamma*I, omega*I]  # gamma * exp(-alpha * t)
        ...
        pred_r = np.multiply(pred_I, gamma - omega)
        pred_confirm = np.add(np.add(pred_I, pred_D), pred_r)

        return pred_S, pred_E, pred_I, pred_D, pred_r, pred_confirm
...
class Learner_SuEIR(Model):
    ...
    def __call__(self, size, params, init, lag=0):
        ...
        def calc_grad(t, y):
            ...
            # new population joining the suspecitible population
            new_pop_in = self.pop_in * (self.pop-self.N) * (np.exp(-0.03 * np.maximum(0, t-self.bias)) + 0.05) 
            return [new_pop_in-beta*S*(E+I)/self.N, beta*S*(E+I)/self.N-sigma*E, mu*E-gamma*I, gamma*I]  # gamma * exp(-alpha * t)
